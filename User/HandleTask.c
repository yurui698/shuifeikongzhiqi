...
u8  factory_gateway_set[255]=            //触摸屏下发出厂网关设定参数需要写入flash0x0805 C000，从256k开始写入，0x0804加1，则增加64k,每次写入必需2Kbyte
{
    ...
    00,01,00,  //网关采集通道0参数设定：采集方式，参数个数，滤波次数；采集方式：00 无采集，01 光照，02 空气温湿度，03 4~20ma，04 二氧化碳
    00,01,00,  //网关采集通道1参数设定; 采集方式：05 大气压力输入，06 开关量输入，07 频率输入，08 0~1Vdc输入，09 RS485 ；见上面采集方式
    00,01,15,  //网关采集通道2参数设定; 触摸屏：功能号：0x06,地址：EF_0C~EF_1D,factory_gateway_set[12]~[29]
    00,01,15,  //网关采集通道3参数设定;
    00,01,00,  //网关采集通道4参数设定;
    31,07,01,	//网关采集通道5参数设定;factory_gateway_set[27]~[29]
        ...
        */
        case 0x51://水肥PID设定开始
            if(pData[1]==0x10)   //触摸屏写功能号0x10只能是字符串或浮点数
            {
                ...
                memcpy(USART3SendTCB,ReceiveData4,13);
