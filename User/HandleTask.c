...
u8  factory_gateway_set[255] =            //触摸屏下发出厂网关设定参数需要写入flash0x0805 C000，从256k开始写入，0x0804加1，则增加64k,每次写入必需2Kbyte
{
    ...
    33,01,00,  //网关采集通道1参数设定; 采集方式：05 大气压力输入，06 开关量输入，07 频率输入，08 0~1Vdc输入，09 RS485 ；见上面采集方式
    00,01,15,  //网关采集通道2参数设定; 触摸屏：功能号：0x06,地址：EF_0C~EF_1D,factory_gateway_set[12]~[29]
    33,01,15,  //网关采集通道3参数设定;
    00,01,00,  //网关采集通道4参数设定;
    34,07,01,	//网关采集通道5参数设定;factory_gateway_set[27]~[29]  33水肥涡轮流量计，34上海帆扬电磁流量计测总管流量
...
static u8 param_fertigation52[36];
...
//    RXENABLE3;//测试
        ...
        Start_timerEx(SF_Flow_Measu_EVT, 3650);
        if (factory_gateway_set[11] != 0)
        {
            u16  logic_ture;
            if ((factory_gateway_set[12] >= 6 && factory_gateway_set[12] <= 7) || factory_gateway_set[12] == 10 || factory_gateway_set[12] == 28)   //开关量及频率信号输入管脚PC0上拉20k电阻；通道0（光照）=10 cps120压力输入
            {
...
//			  memcpy(test,pData,8);
        ...
        */
        case 0x51://水肥PID设定开始
            ...
            {
                SF_cmd_num[pData[3] / 2][0] = 30;     //允许向子站发送50次指令，每发一次减1，收到回复清0
                SF_cmd_num[pData[3] / 2][1] = 30;     //允许向子站发送50次指令，每发一次减1，收到回复清0
...
//							online_slaveID[Query_Wired_WirelesYX]=0x00;//子站有线正在检测通信标志
        ...
        break;
    default:
        Start_timerEx(WG_SENDZZ_EVT, 509);
        break;
    }
}
...
static void wgcollector_data(void)   //传感器采集数据，水肥采用0,1,4三路通道，频率输入转换成流量
{
    ...
    case 0:
        ...
        if (factory_gateway_set[12] == 28)   //表压液位LWP5050GD, LWP5050测量大气压力（通道0 光照）
        {
...
static void uart5_send_cmd(void)
{
    ...
    if (factory_gateway_set[27] == 34)           //水肥机总管流量查询指令，流量信息在rxreport5里接收，需下发
    {
        WriteDataToBuffer(5, (u8 *)uart5_cmd_fydc, 0, 8);    //上海科帆电磁流量计，测水肥机总管流量
        return;
    }
...
static void RxReport5(u8 len, u8 *pData)
{
    if (GetCRC16(pData, len) == 0)
    {
        ...
        if (factory_gateway_set[27] == 34)   //水肥电磁流量计，测总管瞬时流量，单位为T/H ，放大100倍即保留两位小数 注意支管流量为L/H
        {
            if (pData[0] == 0x01 && pData[1] == 0x03)  					//收到传感器返回的数据
            {
                ...
                SF_Flow_Total[SF_Flow_Total_I] = (u16)((*((float *)&SF_Flow_Total_temp))*100);  //2字节流量整型数,放大100倍，如6222表示62.22T/H
...
void SF_Para_Trans() //将收到的0x51水肥PID参数下发到子站地址为SF_SlaveID_0控制器上
{
    ...
    case 0:
        ...
        while (fertigation51.set_float[SF_Qeury_index] == 0 ||SF_cmd_num[SF_Qeury_index][0] == 0)
        {
        ...
        SF_Trans_flg++;
        Start_timerEx(SF_Para_Trans_EVT,100);
        break;
...
{
    Start_timerEx(SF_Flow_Measu_EVT,1000);
    ...
    if(factory_gateway_set[12]==33)//水肥涡轮流量检测计算  //第一路配肥流量下发子站 pc0 H0900
    {
        ...
        SF_collector_temp=20*SF_collector_temp/3;//保留1位小数；所以需要乘以10；（collector_temp*3600*10/450/12）；采样滤波12s
        SF_wgcollector_data_buff[SF_TD_param_num]   =  SF_collector_temp & 0x00ff;           //施肥流量字节低 k=100，b=0
				wgcollector_data_buff[0] = SF_wgcollector_data_buff[SF_TD_param_num];
				SF_TD_param_num++;
        SF_wgcollector_data_buff[SF_TD_param_num]   =  (SF_collector_temp & 0xff00)>>8;     //施肥流量字节高； 实际值=0.6*测量值/100+41
